(() => {
  ...
  const profiles = [
    {
      match: ['고민해볼 문제', '이번 주차에서 연결되는 내용'],
      widths: [68, 32],
      className: 'table--questions'
    },
    {
      match: ['발전 단계', '핵심 방식', '대표적인 변화'],
      widths: [23, 35, 42],
      className: 'table--evolution'
    },
    {
      match: ['학습 방식', '학습할 때 주어지는 정보', '주요 목적', '대표 결과'],
      widths: [17, 25, 34, 24],
      className: 'table--learning-types'
    },
    {
      match: ['확인 질문', '지도학습', '비지도학습', '강화학습'],
      widths: [20, 25, 25, 30],
      className: 'table--learning-compare'
    },
    {
      match: ['단계', '핵심 질문', '결과'],
      widths: [17, 48, 35],
      className: 'table--training-flow'
    },
    {
      match: ['구분', '역할', '확인할 점'],
      widths: [18, 35, 47],
      className: 'table--data-roles'
    },
    {
      match: ['문제 예시', '적합한 학습 방식', '이유'],
      widths: [35, 22, 43],
      className: 'table--problem-method'
    }
  ];
  ...
  if (!document.getElementById(styleId)) {
    ...
    style.textContent = `
      ...
      @media(max-width:760px){
        body[data-week="2"] .notion-content table.week2-balanced-table{min-width:760px}
      }
    `;
